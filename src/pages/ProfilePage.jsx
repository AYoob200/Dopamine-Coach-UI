...
export default function ProfilePage() {
  const { user, logout, language, setLanguage } = useGlobalState();
  ...
  return (
    <div className="max-w-xl mx-auto px-8 py-10">
      <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-8">{isArabic ? ar.pages.settings : 'Settings'}</h2>
      ...
      <div className="space-y-5">
        <div>
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block ml-0.5">{isArabic ? 'الاسم' : 'Name'}</label>
          <input
        ...
        <div>
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block ml-0.5">{isArabic ? ar.auth.email : 'Email'}</label>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-800">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">{isArabic ? ar.settings.language : 'Language'}</h3>
          <div className="py-2">
            <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5 block ml-0.5">
              {isArabic ? ar.settings.languageHint : 'Choose app language and direction'}
            </label>
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="w-full h-11 px-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:border-gray-300 dark:focus:border-gray-600 focus:ring-1 focus:ring-gray-200 dark:focus:ring-gray-700 transition-all"
            >
              {locales.map((locale) => (
                <option key={locale.locale} value={locale.locale}>
                  {`${locale.nativeName} (${locale.direction.toUpperCase()})`}
                </option>
              ))}
            </select>
          </div>
        ...
        <div className="pt-4 border-t border-gray-200 dark:border-gray-800">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">{isArabic ? 'الإشعارات' : 'Notifications'}</h3>
          <div className="flex items-center justify-between py-2">
            <span className="text-sm text-gray-600 dark:text-gray-300">{isArabic ? 'تذكيرات التركيز اليومية' : 'Daily focus reminders'}</span>
        ...
        <div className="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-800">
          <button onClick={() => { logout(); navigate('/login'); }}
            ...
            <LogOut size={16} /> {isArabic ? 'تسجيل الخروج' : 'Sign Out'}
          </button>
